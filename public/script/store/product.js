...
function sliderProduct(sliderId, modalId) {
  ...
  function handleSlideSwipe(event, index) {
    ...
    if (swipeDistance > 250 && index > 0) {
      currentIndex = index - 1;
    } else if (swipeDistance < -250 && index < slides.length - 1) {
      currentIndex = index + 1;
    }
  ...
  modalContent.addEventListener("mousemove", (event) => {
    if (mouseStartX) {
      ...
      if (Math.abs(dragDistance) > 250 && !isScrolling()) {
        event.preventDefault();
        isDragging = true;
      }
    }
  ...
  modalContent.addEventListener("mouseup", (event) => {
    if (isDragging) {
      ...
      if (dragDistance > 250 && currentIndex > 0) {
        navigation("prev");
        updateModalImage();
      } else if (dragDistance < -250 && currentIndex < slides.length - 1) {
        navigation("next");
        updateModalImage();
      }
  ...
  modalContent.addEventListener("touchend", (event) => {
    if (isDragging) {
      ...
      if (swipeDistance > 250 && currentIndex > 0) {
        navigation("prev");
        updateModalImage();
      } else if (swipeDistance < -250 && currentIndex < slides.length - 1) {
        navigation("next");
        updateModalImage();
      }
...
function flyToCart(button) {
  ...
  if (!button.classList.contains("in")) {
    ...
    setTimeout(() => button.classList.remove("in"), 12500);
  }
