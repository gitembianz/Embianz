...
<form class="content" method="POST" enctype="multipart/form-data" action="{{ route('new_products') }}">
  ...
  <nav class="nav--controls">
    ...
    <button class="button button--secondary button--centered" tooltip="Save Product" tooltip-left type="submit">
    ...
    <button class="button button--secondary button--centered" tooltip="Reset Product" tooltip-left type="reset">
  ...
  <section style="height: calc(100% - 107.5px);"  class="tabs__content details__view active">
    ...
    @error('end_date')<span class="error @error('end_date') active @enderror">{{ $message }}</span>@enderror
    @error('sku')<span class="error @error('sku') active @enderror">{{ $message }}</span>@enderror
    @error('ean')<span class="error @error('ean') active @enderror">{{ $message }}</span>@enderror
