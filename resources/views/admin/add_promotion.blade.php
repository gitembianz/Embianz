...
<form class="content" method="POST" enctype="multipart/form-data" action="{{ route('store_promotion') }}">
 ...
 <section style="height: calc(100% - 107.5px);" class="tabs__content details__view active">
  ...
  @if ($vouchers->count() > 0)
   <div class="input__tabs">
    <select name="voucher_id" value="{{ old('voucher_id') }}">
     @foreach ($vouchers as $voucher)
      <option value="{{ $voucher->id }}">{{ $voucher->name }} ({{ $voucher->code }})</option>
     @endforeach
    </select>
    <label>Voucher</label>
   </div>
  ...
  @else
   <a class="button button--fill button--secondary details__long" href="{{ route('new_voucher') }}">Please add a
    voucher first</a>
  @endif
  @error('end_date')
