<div class="content">
    ...
    <div class="aside aside--right" id="sort">
      ...
      <span class="aside--line"></span>
        @foreach ($columns as $column)
          <button class="button button--primary button--long button--flexed button--arrow @if ($orderBy === $column && $orderAsc === '1') active @endif" wire:click="sortBy('{{ $column }}')">
  ...
  {{-- Navigation --}}
  <h1 class="table--name">{{ __('Categories') }} ({{ $categories->count() }})</h1>
  <nav class="nav--controls">
    ...
    {{-- Add New Button --}}
    <a class="button button--secondary button--centered display--desktop" href="{{ route('newcategory') }}">
      <svg>
    ...
    <div class="dropdown dropdown--right display--mobile" id="optional__dropdown">
      ...
      <div class="dropdown__content">
        <div class="dropdown__container">
          ...
          {{-- Add New Button --}}
          <a class="button button--primary button--long button--flexed" href="{{ route('newcategory') }}">
            <svg>
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
              <polyline points="14 2 14 8 20 8"></polyline>
              <line x1="12" y1="18" x2="12" y2="12"></line>
          ...
          @foreach ($selectedColumns as $index => $column)
            @if ($this->showColumn($column))
              <th @if ($index > count($selectedColumns) - 16) class="hidden" @endif>
                <button wire:click="sortBy('{{ $column }}')" class="table--btn @if ($orderBy === $column && $orderAsc === '1') active @endif">
                  {{ $column }}
                  <svg><polyline points="6 9 12 15 18 9"></polyline></svg>
                </button>
              </th>
      ...
      <tbody>
        ...
        @endphp
          @foreach ($categories as $nr => $category)
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($category->id)) active @endif">
              ...
              @foreach ($selectedColumns as $index => $column)
                <td @if ($index > count($selectedColumns) - 16) class="hidden" @endif data-title="{{ $column }}" wire:click="expandRow({{ $nr }})">
                  @if ($column === "name")
              ...
              <td style="border-right: none">
                <button wire:click.prevent="confirmItemRemoval({{ $category->id }})" class="button button--secondary button--sm">
                  <svg>
                    <polyline points="3 6 5 6 21 6"></polyline>
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                  </svg>
