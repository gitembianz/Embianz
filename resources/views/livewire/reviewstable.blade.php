<section class="content">
    ...
    {{-- Navigation --}}
    <h1 class="table--name">{{ __('Reviews') }} ({{ $reviews->total() }})</h1>
            ...
            <tbody>
                ...
                @else
                    @php
                        $i = 0;
                    @endphp
                    @foreach ($reviews as $nr => $review)
                        <tr @if ($loop->last) id="last_record" @endif
                            class="expandable-row @if ($this->isChecked($review->id)) active @endif">
                            <td style="border-left: none" data-title="Check">
                                <div class="checkbox--primary">
                                    <input type="checkbox" value="{{ $review->id }}" id="{{ $review->id }}"
                                        wire:model="checked">
                            ...
                            @foreach ($selectedColumns as $index => $column)
                                <td @if ($index > 1) class="hidden" @endif
                                    ...
                                    @if ($column === 'acronim' || $column === 'comment')
                                        @if ($editindex !== $nr)
                                            {{ $review->$column }}
                                        @else
                                        ...
                                        @if ($editindex !== $nr)
                                            @if ($review->approved)
                                                <div class="checkbox--secondary">
                                                    <input type="checkbox" id="isactive{{ $nr }}" disabled
                                                        checked>
                                                    <label for="isactive{{ $nr }}"></label>
                                                </div>
                                            @else
                                                <div class="checkbox--secondary disabled">
                                                    <input type="checkbox" id="notactive{{ $nr }}"
                                                        disabled>
                                ...
                                <div style="display:flex;">
                                   @if (!$review->approved)
                                    ...
                                    @if ($editindex !== $nr)
                                        ...
                                        <button class="button button--secondary button--sm"
                                            wire:click.prevent="confirmItemRemoval({{ $review->id }})">
                                            <svg>
                                                <polyline points="3 6 5 6 21 6"></polyline>
                                                <path
                                                    d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2">
                                    ...
                                    @else
                                        <button class="button button--secondary button--sm"
                                            wire:click.prevent="saveitem({{ $nr }} , {{ $review->id }})">
                                            <svg>
