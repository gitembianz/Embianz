<section class="content">
  ...
  {{-- Navigation --}}
  <h1 class="table--name">{{ __('Currency') }} ({{ $currencies->count() }})</h1>
    ...
    <table class="expandable-table">
      ...
      <tbody>
        ...
        @php
          $i = 0;
        @endphp
          @foreach ($currencies as $nr => $item)
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($item->id)) active @endif">
              @foreach ($selectedColumns as $index => $column)
                <td @if ($index > count($selectedColumns) - 4) class="hidden" @endif data-title="{{ $column }}" wire:click="expandRow({{ $nr }})">
                  @if($column === 'name')
                    @if ($editeindex !== $index)
                      {{ $item->name }}
                    @else
              ...
              <td style="border-right: none">
                @if ($editeindex !== $index)
                <button class="button button--secondary button--sm" wire:click.prevent="edit({{ $index }}, {{ $item->id }})">
                  <svg>
                ...
                @else
                <button class="button button--secondary button--sm" wire:click.prevent="save({{ $index }} , {{ $item->id }})">
                  <svg>
