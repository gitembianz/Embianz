<section class="content">
 ...
 <nav class="nav--controls">
  <h1 class="table--name">Page: {{ $page->name }}</h1>
  {{-- Refresh Button --}}
  <a class="button button--primary button--centered" tooltip="Back to all page" tooltip-top href="{{ route('pages') }}">
   <svg>
    <polyline points="15 18 9 12 15 6"></polyline>
   </svg>
  </a>
  <a class="button button--primary button--centered" tooltip="Create new page" tooltip-top
   href="{{ route('add_page') }}">
   <svg>
  ...
  @if ($edititem === null)
   <button class="button button--primary button--centered" tooltip="Edit this page" tooltip-left
  ...
  @endif
  <button class="button button--primary button--centered" tooltip="Delete this page" tooltip-left
   wire:click.prevent="confirmItemRemoval({{ $page->id }})">
   <svg>
    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
    <path d="M14 3v4a1 1 0 0 0 1 1h4" />
    <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" />
 ...
 <form style="height: calc(100% - 107.5px);" class="tabs__content details__view active" id="detailsContent">
  ...
  <div class="input__tabs">
   @if ($edititem === null)
    <span class="disabled">{{ $page->name }}</span>
   @else
  ...
  <div class="input__tabs">
   @if ($edititem === null)
    <span class="disabled">{{ $page->route }}</span>
   @else
    <input type="text" placeholder=" " wire:model.defer="record.route">
   @endif
   <label>Route</label>
  </div>
  <div class="input__tabs">
   @if ($edititem === null)
    <span class="disabled">{{ $page->sequence }}</span>
   @else
    <input type="text" placeholder=" " wire:model.defer="record.sequence">
   @endif
   <label>Sequence</label>
  </div>
  <div class="input__tabs">
   @if ($edititem === null)
    <span class="disabled">{{ $page->description }}</span>
   @else
    <input type="text" placeholder=" " wire:model.defer="record.description">
   @endif
   <label>Dscription</label>
  </div>
  <div class="textarea__tabs details__long">
   @if ($edititem === null)
    <span class="disabled">{{ $page->content }}</span>
   @else
    <textarea type="text" placeholder=" " name="content" wire:model.defer="record.content"></textarea>
   @endif
  ...
  {{-- page List Create date / time --}}
  <div class="input__tabs">
   <span class="disabled">{{ $page->created_at }}</span>
   <label>Create date / time</label>
  </div>

  {{-- page List Create by --}}
  <div class="input__tabs">
   <span class="disabled">{{ $page->created_by }}</span>
   <label>Create by</label>
  </div>

  {{-- page List Updated date / time --}}
  <div class="input__tabs">
   <span class="disabled">{{ $page->updated_at }}</span>
   <label>Updated date / time</label>
  </div>

  {{-- page List Last modified by --}}
  <div class="input__tabs">
   <span class="disabled">{{ $page->last_modified_by }}</span>
