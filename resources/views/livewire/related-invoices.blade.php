                    ...
                    @foreach ($invoices as $index => $invoice)
                        <tr @if ($loop->last) id="last_record" @endif
                            ...
                            <td style="border-left: none" data-title="Check">
                                <div class="checkbox--primary">
                                    <input type="checkbox" value="{{ $invoice->id }}" id="{{ $invoice->id }}"
                                        wire:model="checked">
                            ...
                            @if ($this->showColumn('Order'))
                                <td wire:click="expandRow({{ $index }})">
                                    @if ($invoice->order_id === null)
                                        <span class="badge badge--secondary">No Order</span>
                                    @else
                                        <a
                                            href="{{ route('show_order', ['id' => $invoice->order_id]) }}">{{ $invoice->order->name }}</a>
                                    @endif
                                </td>
                            @endif
                            @if ($this->showColumn('Account'))
                                <td wire:click="expandRow({{ $index }})">
                                    <a
                                        href="{{ route('show_account', ['id' => $invoice->account_id]) }}">{{ $invoice->account->name }}</a>
                                </td>
                            @endif
                            @if ($this->showColumn('Date'))
