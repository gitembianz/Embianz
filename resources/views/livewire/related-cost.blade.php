<div class="accordion @if ($showrelated) active @endif">
 ...
 <div class="accordion__header">
  <button
   ...
   {{ __('Product average cost ') }}({{ $costs->total() }})
   <svg>
      ...
      <tr @if ($loop->last) id="last_record" @endif class="expandable-row">
       ...
       @foreach ($selectedColumns as $column)
        <td @if ($index > count($selectedColumns) - 17) class="hidden" @endif data-title="{{ $column }}">
         @if ($column === 'product_id')
          <a href="{{ route('show_product', ['id' => $cost->$column]) }}">{{ $cost->product->name }}</a>
         @elseif ($column === 'price')
          @if ($editindex !== $index)
           {{ $cost->$column }}
          @else
           <input type="number" min="0" required class="input__searchable"
            wire:model.defer="record.{{ $index }}.price">
          @endif
         @elseif ($column === 'cost')
          @if ($editindex !== $index)
           {{ $cost->$column }}
          @else
           <input type="number" required class="input__searchable" wire:model.defer="record.{{ $index }}.cost">
          @endif
         @else
          {{ $cost->$column }}
         @endif
       ...
       <td>
        @if ($editindex !== $index)
         <div style="display: flex;">
          <button class="button button--secondary button--sm"
           wire:click.prevent="edititem({{ $index }}, {{ $cost->id }})">
           <svg>
        ...
        @else
         <div style="display: flex;">
          <button class="button button--secondary button--sm"
           wire:click.prevent="saveitem({{ $index }},{{ $cost->id }})">
           <svg>
