<section class="content">
  ...
  {{-- Navigation --}}
  <h1 class="table--name">{{ __('Specifications') }} ({{ $specs->count() }})</h1>
  <nav class="nav--controls">
    ...
    {{-- Add specification --}}
    <a class="button button--secondary button--centered display--desktop" tooltip="Add new specification" tooltip-top href="{{ route('new_spec') }}">
      <svg>
    ...
    <div class="dropdown dropdown--right display--mobile" id="optional__dropdown">
      ...
      <div class="dropdown__content">
        <div class="dropdown__container">
          ...
          <a class="button button--primary button--fill button--flexed" href="{{ route('new_spec') }}">
            <svg>
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
              <polyline points="14 2 14 8 20 8"></polyline>
              <line x1="12" y1="18" x2="12" y2="12"></line>
        ...
        @endphp
          @foreach ($specs as $nr => $item)
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($item->id)) active @endif"  >
              ...
              @foreach ($selectedColumns as $index => $column)
                <td @if ($index > count($selectedColumns) - 7) class="hidden" @endif data-title="{{ $column }}" wire:click="expandRow({{ $nr }})">
                  @if ($column === 'name')
