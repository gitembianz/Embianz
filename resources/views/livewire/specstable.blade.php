				...
				<thead>
						<tr>
								...
								@if ($this->showColumn('Id'))
										<th class="cursor-p" @if ($orderBy === 'id' && $orderAsc === '1') data-symbol="up" @else data-symbol="down" @endif
                    ...
                    data-symbol="down" @endif
								...
								@if ($this->showColumn('Unit'))
										<th class="cursor-p" @if ($orderBy === 'um' && $orderAsc === '1') data-symbol="up" @else data-symbol="down" @endif
						...
						@foreach ($specs as $index => $spec)
								<tr class="@if ($this->isChecked($spec->id)) th_checked @endif">
												...
												<td data-title="Name">
														...
														@if ($indexspec !== $index)
																<div class="cursor-p" wire:click.prevent="edititem({{ $index }})">
																		{{ $spec->name }}</div>
														@else
																<input type="text" class="table-edit wid-1" wire:model.defer="specss.{{ $index }}.name">
														@endif
												</td>
										@endif
										@if ($this->showColumn('Unit'))
												<td data-title="Unit">
														@if ($indexspec !== $index)
																<div class="cursor-p" wire:click.prevent="edititem({{ $index }})">
																		{{ $spec->um }}</div>
														@else
																<input type="text" class="table-edit wid-1" wire:model.defer="specss.{{ $index }}.um">
														@endif
												</td>
										@endif
										@if ($this->showColumn('Group'))
										...
										<td data-title="Action">
												...
												@else
														<button class="edit" wire:click.prevent="saveitem({{ $index }} , {{ $spec->id }})">
																<svg>
