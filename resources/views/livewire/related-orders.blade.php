<div class="accordion @if ($showrelated) active @endif" id="accordion__addresses">
  ...
  <div class="accordion__body">
    ...
    <nav class="nav--controls">
      ...
      {{-- IF CHECKED --}}
      <div class="dropdown dropdown--right" @if (!$checked) style="display:none;" @endif id="checkorder__dropdown">
        {{-- Dropdown Button --}}
        <button class="button button--secondary button--centered button--long" tooltip="Actions with checked" tooltip-top id="checkorder__open">
      ...
      <thead>
        <tr>
          ...
          @if ($this->showColumn('Quantity Amount'))
            <th class="hidden">
              <button wire:click="sortBy('quantity_amount')" class="table--btn @if ($orderBy === $column && $orderAsc === '1') active @endif">
                Quantity Amount
                <svg><polyline points="6 9 12 15 18 9"></polyline></svg>
              </button>
            </th>
          ...
          @if ($this->showColumn('Delivery Method'))
            <th class="hidden">
              <button wire:click="sortBy('delivery_method')" class="table--btn @if ($orderBy === $column && $orderAsc === '1') active @endif">
                Delivery Method
                <svg><polyline points="6 9 12 15 18 9"></polyline></svg>
              </button>
            </th>
      ...
      <tbody>
        ...
        @else
          @foreach ($orders as $index => $order)
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($order->id)) active @endif" >
              ...
              @endif
              <td>
                <button wire:click.prevent="confirmItemRemoval({{ $order->id }})" class="button button--secondary button--sm">
                  <svg>
                    <polyline points="3 6 5 6 21 6"></polyline>
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                  </svg>
