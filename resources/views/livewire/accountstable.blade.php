<section class="content">
  ...
  {{-- Navigation --}}
  <h1 class="table--name">{{ __('Accounts') }} ({{ $accounts->count() }})</h1>
        ...
        <tr>
          ...
          @foreach ($selectedColumns as $index => $column)
            @if ($this->showColumn($column))
              <th @if ($index > count($selectedColumns) - 11) class="hidden" @endif>
      ...
      <tbody>
        ...
        @endphp
          @foreach ($accounts as $nr => $account)
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($account->id)) active @endif"  >
              ...
              @foreach ($selectedColumns as $index => $column)
                <td @if ($index > count($selectedColumns) - 11) class="hidden" @endif data-title="{{ $column }}" wire:click="expandRow({{ $nr }})">
                  @if ($column === "name")
              ...
              <td style="border-right: none">
                <button wire:click.prevent="confirmItemRemoval({{ $account->id }})" class="button button--secondary button--sm">
                  <svg>
                    <polyline points="3 6 5 6 21 6"></polyline>
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                  </svg>
