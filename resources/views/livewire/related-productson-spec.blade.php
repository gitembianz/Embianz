<div>
	...
	<div class="accordion">
		<div class="accordion__btn-flex">
			<button class="accordion__btn" wire:click.prevent="@if ($showrelatedprods === false) $set('showrelatedprods', true) @else $set('showrelatedprods', false) @endif">
			...
			<div class="modal" id="modalelements" style="display: block">
				<div class="modal-content modal--tabel">
					...
					<div class="panel__header">
						...
						<input class="panel__header--input panel__header--checked" wire:click.prevent="saveprod()" type="button" value="Save">
					</div>
					{{-- Table --}}
							...
							<tbody>
								@foreach ($prod as $index => $pro)
									<tr wire:key="spec-row-{{ $index }}">
													...
													<ul class="table__drop--list">
														@if (count($addprods) >= 1)
															@foreach ($addprods as $pr)
																<li class="table__drop--item" wire:click.prevent="selectitem({{ $index }}, {{ $pr->id }}, '{{ $pr->name }}')">
																	{{ $pr->name }}
																</li>
															@endforeach
														@else
										...
										<td data-title="Value">
											<input type="text" required class="table__drop--input" wire:model.defer="prod.{{ $index }}.product.value">
										</td>
			...
			<div class="modal" id="modalelements" style="display: block">
				<div class="modal-content modal--tabel">
					...
					<div class="panel__header">
						...
						<input class="panel__header--input panel__header--checked" wire:click.prevent="confirmmultiple()" type="button" value="Save">
					</div>
					{{-- Table --}}
					<div style="overflow: auto; position: relative; background: white;height: 100%">
						<table class="table table-top">
								...
								@foreach ($relatedprods as $index => $prod)
									...
									<tr class="@if ($this->isChecked($prod->id)) table__row--selected @endif">
													...
													@if ($allow)
														<div class="table__drop" style="position: relative">
															...
															<ul class="table__drop--list">
																...
																@else
																	@foreach ($addprods as $pr)
																		<li class="table__drop--item" wire:click.prevent="select({{ $pr->id }})">
																			{{ $pr->name }}
																		</li>
																	@endforeach
																@endif
										...
										<td data-title="Action">
											<div class="table__buttons">
												@if ($editedrow !== $index)
													<button class="edit" wire:click.prevent="editprod({{ $prod->id }}, {{ $prod->product->id }}, {{ $index }})">
														<svg>
															<path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z">
															</path>
														</svg>
													</button>
													<button class="delete" wire:click.prevent="confirmRemoval({{ $prod->id }})">
														<svg>
												...
												@else
													<button class="edit" wire:click.prevent="confirmprod({{ $index }},{{ $prod->id }})">
														<svg>
