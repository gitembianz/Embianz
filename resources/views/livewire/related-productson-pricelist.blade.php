            ...
            <tbody>
              ...
              @foreach ($prod as $index => $pro)
                <tr class="expandable-row">
                        ...
                        <div class="list__searchable">
                          @if (count($addprods) >= 1)
                            @foreach ($addprods as $pr)
                              <button class="item__searchable" wire:click.prevent="selectitem({{ $index }}, {{ $pr->id }}, '{{ $pr->name }}')">
                                {{ $pr->name }}
                              </button>
                            @endforeach
                          @else
                  ...
                  <td class="hidden">
                    <input type="number" required class="input button--fill button--xs" wire:model.defer="prod.{{ $index}}.product.discount">
                  </td>
                  <td class="hidden" style="width: auto !important;">
                    <input type="number" required class="input button--fill button--xs" wire:model.defer="prod.{{ $index}}.product.vat">
                  </td>
                </tr>
                  ...
                  <td colspan="3">
                    <div class="details">
                      ...
                      <p>
                        <bold>Value</bold>
                        <div class="searchable">
                          <input class="input__searchable" type="text" required wire:model.defer="prod.{{ $index }}.product.value">
                        </div>
                      </p>
                      <p>
                        <bold>Discount</bold>
            ...
            <tbody>
              ...
              @foreach ($prod as $index => $pro)
                <tr class="expandable-row">
                  ...
                  <td class="hidden">
                    <input type="text" required class="input button--fill button--xs"  wire:model.defer="prod.{{ $index }}.product.discount">
                  </td>
                  <td class="hidden" style="width: auto;">
                    <input type="text" required class="input button--fill button--xs"   wire:model.defer="prod.{{ $index }}.product.vat">
                  </td>
                  <td>
                    <div style="display: flex;">
                      @if ($index == $row - 1)
              ...
              <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($prod->id)) active @endif">
                  ...
                  <td>
                    ...
                    @else
                      @if ($allow)
                        <div class="searchable active">
                          <!-- Header -->
                          <input class="input__searchable" type="text" wire:model.debounce.300ms="searchadd" placeholder="Search..">
                ...
                @if ($this->showColumn('Discount'))
                  <td class="hidden">
                    ...
                    @else
                    <div class="searchable">
                      <input type="number"class="input__searchable" wire:model.defer="product.{{ $index }}.discount">
                    </div>
                    @endif
                  </td>
                @endif
                ...
                @if ($this->showColumn('VAT'))
                  <td class="hidden">
                    ...
                    @else
                    <div class="searchable">
                      <input type="number"class="input__searchable"  wire:model.defer="product.{{ $index }}.vat">
                    </div>
                    @endif
                  </td>
                @endif
                ...
                <td>
                  <div style="display: flex;">
                    @if ($editedrow !== $index)
                        <button class="button button--secondary button--sm" wire:click.prevent="edititem({{ $prod->id }}, {{ $prod->product->id }}, {{ $index }})">
                          <svg>
                          <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z">
                          </path>
                          </svg>
                        </button>
                        <button class="button button--secondary button--sm" wire:click.prevent="confirmItemRemoval({{ $prod->id }})">
                          <svg>
                            <polyline points="3 6 5 6 21 6"></polyline>
                            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                          </svg>
                    ...
                    @else
                        <button class="button button--secondary button--sm" wire:click.prevent="confirmitem({{ $index }},{{ $prod->id }})">
                          <svg>
