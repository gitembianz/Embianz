<div class="accordion @if ($showrelatedprice) active @endif" >
                          ...
                          <div class="list__searchable">
                            @if (count($addprices) >= 1)
                              @foreach ($addprices as $pri)
                                <button class="item__searchable" wire:click.prevent="selectitem({{ $index }}, {{ $pri->id }}, '{{ $pri->name }}')">
                                  {{ $pri->name }}
                                </button>
                              @endforeach
                            @else
  ...
  {{-- Accordion Header --}}
    <div class="accordion__header">
      <button class="button button--flexed button--fill button--primary @if ($showrelatedprice) button--secondary active @endif" wire:click.prevent="@if ($showrelatedprice === false) $set('showrelatedprice', true) @else $set('showrelatedprice', false) @endif">
      ...
      <table class="expandable-table">
        ...
        <tbody>
          ...
          @else
            ...
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($prices->id)) active @endif">
              ...
              <div style="display: flex;">
                  @if ($editedrow !== $nr)
                    ...
                    <button class="button button--secondary button--sm" wire:click.prevent="confirmItemRemoval({{ $prices->id }})">
                    <svg>
                      <polyline points="3 6 5 6 21 6"></polyline>
                  ...
                  @else
                    <button class="button button--secondary button--sm" wire:click.prevent="confirmitem({{ $index }},{{ $prices->id }})">
                    <svg>
