<div class="accordion @if ($showrelateitems) active @endif">
    ...
    <aside>
        <div class="background background--center @if ($linksingle || $linkmultiple) active @endif"
            style="z-index: 1100;">
        </div>
    ...
    <aside>
        ...
        <div class="aside aside--table @if ($showTable) active @endif">
            ...
            <nav class="nav--controls" style="margin-top: 10px">
                <input class="input input--long" type="text" wire:model.debounce.300ms="searchadd"
                    placeholder="Search...">
            ...
            @elseif($selectPageadd)
                <button class="button button--fill button--secondary" style="margin-top: 10px;"
                    wire:click.prevent="selectAlladd">
            ...
            <div class="table" style="height: calc(100% - 60px);">
                        ...
                        @foreach ($articles as $index => $article)
                            <tr class="expandable-row">
                                <td style="border-left: none" data-title="Check">
                                    <div class="checkbox--primary">
                                        <input type="checkbox" value="{{ $article->id }}" id="{{ $article->id }}"
                                ...
                                <td>
                                    <button class="button button--secondary button--sm"
                                        wire:click.prevent="confirmItemLink({{ $article->id }})">
                                        <svg>
                                            <path
                ...
                {{-- Load More Manual --}}
                @if ($loadAmount < $articles->total())
                    <button class="button button--secondary button--fill" style="margin-top: 10px;"
                        wire:click="loadMore">
                        Load more
                    </button>
                @endif
    ...
    <div class="accordion__header">
        <button
            ...
            {{ __('Articles ') }}({{ $relatedarticles->total() }})
            <svg>
    ...
    <div class="accordion__body">
        ...
        <table class="expandable-table">
            <thead>
                <tr>
                    ...
                    @endif
                    @if ($this->showColumn('Article Name'))
                        <th>
                            <button class="table--btn">
                                Article Name
                                <svg>
                    ...
                    @endif
                    @if ($this->showColumn('Article Description'))
                        <th class="hidden">
                            <button class="table--btn">
                                Article Description
                                <svg>
                        ...
                        <tr class="expandable-row @if ($this->isChecked($related->id)) active @endif">
                            ...
                            @if ($this->showColumn('Category Name'))
                                <td data-title="Name" wire:click="expandRow({{ $index }})">
                                    <a href="{{ route('show_articlecategory', ['id' => $related->category->id]) }}">
                                        {{ strip_tags($category->name) }}</a>
                                </td>
                            @endif
                            @if ($this->showColumn('Article Name'))
                                <td data-title="Name" wire:click="expandRow({{ $index }})">
                                    <a href="{{ route('show_article', ['id' => $related->article->id]) }}">
                            ...
                            @if ($this->showColumn('Article Description'))
                                <td class="hidden" wire:click="expandRow({{ $index }})">
                                    {{ $related->article->short_description }}
                                </td>
                            @endif
                            @if ($this->showColumn('Article is active?'))
                                <td class="hidden" wire:click="expandRow({{ $index }})">
                                    @if ($related->article->active)
                                        <div class="checkbox--secondary disabled">
                        ...
                        <tr class="details-row  @if ($rind === $i) active @endif">
                            <td colspan="{{ count($columns) + 2 }}">
                                <div class="details">
                                    ...
                                    @if ($this->showColumn('Article Description'))
                                        <p>
                                            <bold>Article Description</bold>
                                            {{ $related->article->short_description }}
                                        </p>
                                    @endif
                                    @if ($this->showColumn('Article is active?'))
                                        <p>
                                            <bold>Article is active?</bold>
                                            @if ($related->article->active)
                                                <div class="checkbox--secondary disabled">
        ...
        {{-- Load More --}}
        @if ($loadAmount < $relatedarticles->total())
