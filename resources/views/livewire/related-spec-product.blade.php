<div class="accordion @if ($showrelatedspecs) active @endif" id="accordion__addresses">
            ...
            <tbody>
              ...
              @foreach ($specsAndValues as $index => $specAndValue)
                <tr class="expandable-row">
                          ...
                          <div class="list__searchable">
                            @if (count($addspecs) >= 1)
                              @foreach ($addspecs as $spec)
                                <button class="item__searchable" wire:click.prevent="selectSpec({{ $index }}, {{ $spec->id }}, '{{ $spec->name }}')">
                                  {{ $spec->name }} ({{ $spec->um }})
                                </button>
                              @endforeach
                            @else
                  ...
                  <td class="hidden">
                    <input type="text" required class="input button--fill button--xs"  wire:model.defer="specsAndValues.{{ $index }}.spec.value">
                  </td>
                  <td class="hidden" style="width: auto;">
                    <input type="text" required class="input button--fill button--xs" wire:model.defer="specsAndValues.{{ $index }}.spec.sequence">
                  </td>
                </tr>
  ...
  {{-- Accordion Header --}}
    <div class="accordion__header">
      <button class="button button--flexed button--fill button--primary @if ($showrelatedspecs) button--secondary active @endif" wire:click.prevent="@if ($showrelatedspecs === false) $set('showrelatedspecs', true) @else $set('showrelatedspecs', false) @endif">
        ...
        <tbody>
          ...
          @else
            @foreach ($relatedspecs as $index => $spec)
              <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($spec->id)) active @endif">
                ...
                @if ($this->showColumn("Value"))
                  <td class="hidden" wire:click="expandRow({{ $index }})">
                    ...
                    @else
                      <div class="searchable">
                        <input class="input__searchable" type="text" required  wire:model="specification.{{ $index }}.value">
                      </div>
                    @endif
                  </td>
                @endif
                @if ($this->showColumn("Sequence"))
                  <td class="hidden" wire:click="expandRow({{ $index }})">
                    ...
                    @else
                      <div class="searchable">
                        <input class="input__searchable" type="text" required  wire:model="specification.{{ $index }}.sequence">
                      </div>
                    @endif
                  </td>
                @endif
                ...
                <td>
                  <div style="display: flex;">
                    @if ($editedrow !== $index)
                      <button class="button button--secondary button--sm" wire:click.prevent="editspec({{ $spec->id }}, {{ $spec->spec->id }}, {{ $index }})">
                        <svg>
                        <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z">
                        </path>
                        </svg>
                      </button>
                      <button class="button button--secondary button--sm" wire:click.prevent="confirmItemRemoval({{ $spec->id }})">
                        <svg>
                          <polyline points="3 6 5 6 21 6"></polyline>
                          <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                        </svg>
                    ...
                    @else
                      <button class="button button--secondary button--sm" wire:click.prevent="confirmspecs({{ $index }},{{ $spec->id }})">
                        <svg>
