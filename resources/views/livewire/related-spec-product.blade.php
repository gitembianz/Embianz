<div>
	...
	<div class="accordion">
		<div class="accordion__btn-flex">
			<button class="accordion__btn" wire:click.prevent="@if ($showrelatedspecs === false) $set('showrelatedspecs', true) @else $set('showrelatedspecs', false) @endif">
			...
			<div class="modal" id="modalelements" style="display: block">
				<div class="modal-content modal--tabel">
					...
					<div class="panel__header">
						...
						<input class="panel__header--input panel__header--checked" wire:click.prevent="savespecs()" type="button" value="Save">
					</div>
					{{-- Table --}}
								...
								@foreach ($specsAndValues as $index => $specAndValue)
									<tr>
										...
										<td class="wid-2" data-title="Specification">
											@if ($specAndValue["allow"])
												<div class="table__drop" style="position: relative">
													<input class="table__drop--input" wire:model.debounce.300ms="searchadd" placeholder="Search..." type="search">
													<ul class="table__drop--list cursor-p">
														@if (count($addspecs) >= 1)
															@foreach ($addspecs as $spec)
																<li class="table__drop--item" wire:click.prevent="selectSpec({{ $index }}, {{ $spec->id }}, '{{ $spec->name }}')">
																	{{ $spec->name }} ({{ $spec->um }})
																</li>
															@endforeach
														@else
										...
										<td class="wid-1" data-title="Value">
											<input type="text" required class="table__drop--input" wire:model.defer="specsAndValues.{{ $index }}.spec.value">
										</td>
			...
			<div class="modal" id="modalelements" style="display: block">
				<div class="modal-content modal--tabel">
					...
					<div class="panel__header">
						...
						<input class="panel__header--checked panel__header--input" wire:click.prevent="confirmspecsmultiple()" type="button" value="Save">
					</div>
					<div style="overflow-y: auto; position: relative; background: white; height: 100%">
						<table class="table table-top">
							<thead>
											...
											@if ($specAndValue["allow"])
												<div class="table__drop" style="position: relative">
													...
													<ul class="table__drop--list">
														...
														@else
															@foreach ($addspecs as $spec)
																<li class="table__drop--item" style="cursor: pointer" wire:click.prevent="selectSpec({{ $index }}, {{ $spec->id }}, '{{ $spec->name }}')">
																	{{ $spec->name }} ({{ $spec->um }})
																</li>
															@endforeach
														@endif
				...
				@if ($item->product_specs()->count() > 0)
					...
					<div class="modal" id="confirmationmodalspec">
						<div class="modal-content">
							...
							<input class="modal-content-btn delete" type="button" onclick="document.getElementById('confirmationmodalspec').style.display='none'" value="Cancel">
							<span class="modal-content-btn delete" onclick="document.getElementById('confirmationmodalspec').style.display='none'">
								<svg>
									<line x1="18" y1="6" x2="6" y2="18">
									</line>
									<line x1="6" y1="6" x2="18" y2="18">
								...
								@foreach ($relatedspecs as $index => $spec)
									...
									<tr class="@if ($this->isChecked($spec->id)) table__row--selected @endif">
												...
												@else
													@if ($allow)
														<div class="table__drop" style="position: relative">
															...
															<svg wire:click.prevent="denny()" style="background: #35424b;position: absolute;top: 50%;transform: translateY(-50%);right: 10px;border-radius: 5px;padding: 5px;opacity: .7;stroke: white;" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x">
																<line x1="18" y1="6" x2="6" y2="18"></line>
																<line x1="6" y1="6" x2="18" y2="18"></line>
															</svg>
														</div>
										...
										@if ($this->showColumn("Value"))
											<td>
												@if ($editedrow !== $index)
													{{ $spec->value }}
												@else
													<input type="text" required class="table__edit" wire:model="specification.{{ $index }}.value">
												@endif
											</td>
										@endif
										@if ($this->showColumn("Created At"))
										...
										<td data-title="Action">
											<div class="table__buttons">
												@if ($editedrow !== $index)
													<button class="edit" wire:click.prevent="editspec({{ $spec->id }}, {{ $spec->spec->id }}, {{ $index }})">
														<svg>
															<path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z">
															</path>
														</svg>
