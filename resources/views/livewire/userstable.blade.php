<section class="content">
 ...
 {{-- Navigation --}}
 <h1 class="table--name">{{ __('Users') }} ({{ $users->count() }})</h1>
 <nav class="nav--controls">
  ...
  {{-- Add New Button --}}
  <a href="{{ route('add_user') }}" class="button button--primary button--centered display--desktop"
   tooltip="Add new user" tooltip-top>
   <svg>
  ...
  <div class="dropdown dropdown--right display--mobile">
   ...
   <div class="dropdown__content">
    <div class="dropdown__container">
     ...
     {{-- Add New Button --}}
     <a class="button button--primary button--fill button--flexed" href="{{ route('add_user') }}">
      <svg>
       <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
       <polyline points="14 2 14 8 20 8"></polyline>
       <line x1="12" y1="18" x2="12" y2="12"></line>
    ...
    @else
     ...
     @foreach ($users as $nr => $user)
      <tr @if ($loop->last) id="last_record" @endif
       class="expandable-row @if ($this->isChecked($user->id)) active @endif">
       <td style="border-left: none" data-title="Check">
        <div class="checkbox--primary">
         <input type="checkbox" value="{{ $user->id }}" id="{{ $user->id }}" wire:model="checked">
       ...
       <td style="border-right: none">
        <button wire:click.prevent="confirmItemRemoval({{ $user->id }})"
