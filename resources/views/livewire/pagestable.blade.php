        ...
        <div class="dropdown dropdown--right">
            ...
            <div class="dropdown__content">
                <div class="dropdown__container">
                    ...
                    <a class="button button--primary button--fill button--flexed" href="{{ route('add_page') }}">
                        <svg>
                    ...
                    @foreach ($pages as $nr => $page)
                        <tr @if ($loop->last) id="last_record" @endif
                            ...
                            <td style="border-left: none" data-title="Check">
                                <div class="checkbox--primary">
                                    <input type="checkbox" value="{{ $page->id }}" id="{{ $page->id }}"
                                        wire:model="checked">
                            ...
                            @foreach ($selectedColumns as $index => $column)
                                <td @if ($index > 1) class="hidden" @endif
                                    ...
                                    @if ($column === 'name')
                                        <a
                                            href="{{ route('show_page', ['id' => $page->id]) }}">{{ $page->name }}</a>
                            ...
                            <td style="border-right: none">
                                <button wire:click.prevent="confirmItemRemoval({{ $page->id }})"
                        ...
                        <tr class="details-row  @if ($row === $i) active @endif">
                            <td colspan="4">
                                <div class="details">
                                    @foreach ($selectedColumns as $index => $column)
                                        @if ($index > 1 && $column != 'content')
                                            @if ($column === 'display_in_footer' || $column === 'active')
                                                <p>
                                            ...
                                            @else
                                                <p>
                                                    ...
                                                    @if ($column === 'name')
                                                        <a
                                                            href="{{ route('show_page', ['id' => $page->id]) }}">{{ $page->$column }}</a>
                                                    @elseif($column === 'content')
                                                        <span class="show-less">
                                                            {!! $page->$column !!}
                                                        </span>
