                        ...
                        <tr @if ($loop->last) id="last_record" @endif
                            ...
                            <td style="border-left: none" data-title="Check">
                                <div class="checkbox--primary">
                                    <input type="checkbox" value="{{ $cart->id }}" id="{{ $cart->id }}"
                                        wire:model="checked">
                            ...
                            @foreach ($selectedColumns as $index => $column)
                                <td @if ($index > 1) class="hidden" @endif
                                    ...
                                    @if ($column === 'name')
                                        <a
                                            href="{{ route('show_cart', ['id' => $cart->id]) }}">{{ $cart->name }}</a>
                                    @elseif($column === 'session_id')
                                        <a
                                            href="{{ route('show_session', ['id' => $cart->session_id]) }}">{{ $cart->$column }}</a>
                                    ...
                                    @elseif ($column === 'order_id')
                                        @if ($cart->order_id)
                                            <a
                                                href="{{ route('show_order', ['id' => $cart->order_id]) }}">{{ optional($cart->order)->name }}</a>
                                        @endif
                                    @else
                                        {{ $cart->$column }}
                                    @endif
                                            ...
                                            @elseif ($column === 'order_id')
                                                @if ($cart->order_id)
                                                    <p>
                                                        <bold>{{ $column }}:</bold>
                                                        <a
                                                            href="{{ route('show_order', ['id' => $cart->order_id]) }}">{{ $cart->order->name }}</a>
                                                    </p>
                                                @endif
                                            @else
                                                <p>
