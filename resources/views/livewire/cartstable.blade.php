<section class="content">
  ...
  {{-- Navigation --}}
  <h1 class="table--name">{{ __('Carts') }} ({{ $carts->count() }})</h1>
      ...
      <tbody>
        ...
        @endphp
          @foreach ($carts as $nr => $cart)
            <tr @if ($loop->last) id="last_record" @endif class="expandable-row @if ($this->isChecked($cart->id)) active @endif"  >
              ...
              @foreach ($selectedColumns as $index => $column)
                <td @if ($index > count($selectedColumns) - 14) class="hidden" @endif data-title="{{ $column }}" wire:click="expandRow({{ $nr }})">
                @if ($column === 'name')
              ...
              <td style="border-right: none">
                <button wire:click.prevent="confirmItemRemoval({{ $cart->id }})" class="button button--secondary button--sm">
                  <svg>
                    <polyline points="3 6 5 6 21 6"></polyline>
                    <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                  </svg>
