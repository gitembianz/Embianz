            ...
            @if ($edititem === null)
                <span class="disabled">
                    @if ($supplier->exchange_id != null)
                        {{ $supplier->exchange->base_currency->name }} /
                        {{ $supplier->exchange->quote_currency->name }} date -
                                ...
                                @if (count($exchanges) >= 1)
                                    @foreach ($exchanges as $exchange)
                                        <button class="item__searchable"
                                            wire:click.prevent="selectitem({{ $exchange->id }})">
                                            {{ $exchange->base_currency->name }} /
                                            {{ $exchange->quote_currency->name }} date - {{ $exchange->date }}
