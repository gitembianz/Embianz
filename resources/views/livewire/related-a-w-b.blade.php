<div class="accordion @if ($showrelated) active @endif">
 ...
 {{-- preview sistem --}}
 <aside>
  <div class="background background--center @if ($previewurl) active @endif"></div>
  <div class="aside aside--table @if ($previewurl) active @endif">
    ...
    @else
     @foreach ($awbs as $index => $awb)
      <tr @if ($loop->last) id="last_record" @endif
       class="expandable-row @if ($this->isChecked($awb->id)) active @endif">
       <td style="border-left: none" data-title="Check">
        <div class="checkbox--primary">
         <input type="checkbox" value="{{ $awb->id }}" id="{{ $awb->id }}" wire:model="checked">
         <label for="{{ $awb->id }}"></label>
        </div>
       </td>
       @if ($this->showColumn('Id'))
        <td wire:click="expandRow({{ $index }})">{{ $awb->id }}</td>
       @endif
       @if ($this->showColumn('Order'))
        <td wire:click="expandRow({{ $index }})">
         <a href="{{ route('show_order', ['id' => $awb->order_id]) }}">{{ $awb->order->name }}</a>
        </td>
       @endif
       @if ($this->showColumn('Date'))
       ...
       <td style="border-right: none">
        <div style="display:flex;">
         <button wire:click.prevent="confirmItemRemoval({{ $awb->id }})"
         ...
         <button wire:click.prevent="downloadawb({{ $awb->id }})" class="button button--secondary button--sm">
          <svg>
         ...
         <button wire:click.prevent="previewawb({{ $awb->id }})" class="button button--secondary button--sm">
          <svg>
