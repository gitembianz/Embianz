<section class="content">
 ...
 <nav class="nav--controls">
  ...
  {{-- Add Voucher --}}
  <a class="button button--secondary button--centered display--desktop" tooltip="Add new voucher" tooltip-top
   href="{{ route('new_voucher') }}">
   <svg>
         ...
         @foreach ($selectedColumns as $index => $column)
          @if ($index >= count($selectedColumns) - 9)
           ...
           @elseif ($column === 'status_id')
            @if ($editindex !== $nr)
             <p>
              <bold>{{ str_replace('_id', '', $column) }}:</bold> {{ $voucher->status->name }}
             </p>
            @else
             <p>
              <bold>{{ $column }}:</bold>
