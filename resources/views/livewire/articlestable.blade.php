<section class="content">
    ...
    {{-- Navigation --}}
    <h1 class="table--name">{{ __('Articles') }} ({{ $articles->total() }})</h1>
        ...
        <div class="dropdown dropdown--right">
            ...
            <div class="dropdown__content">
                <div class="dropdown__container">
                    ...
                    {{-- Add New Button --}}
                    <a class="button button--primary button--fill button--flexed" href="{{ route('newarticle') }}">
                        <svg>
                ...
                @else
                    ...
                    @foreach ($articles as $nr => $article)
                        <tr @if ($loop->last) id="last_record" @endif
                            class="expandable-row @if ($this->isChecked($article->id)) active @endif">
                            ...
                            @foreach ($selectedColumns as $index => $column)
                                <td @if ($index > 1) class="hidden" @endif
                                    ...
                                    @if ($column === 'name')
                                        <a
                                            href="{{ route('show_article', ['id' => $article->id]) }}">{{ $article->name }}</a>
                                    @elseif ($column === 'active')
                                        @if ($article->$column)
                                            <div class="checkbox--secondary disabled">
                                                <input type="checkbox" id="disabled2{{ $nr }}" disabled checked>
                                                <label id="disabled2{{ $nr }}"></label>
                                            </div>
                                        @else
                                            <div class="checkbox--secondary disabled">
                                                <input type="checkbox" id="disabled1{{ $nr }}" disabled>
                                                <label for="disabled1{{ $nr }}"></label>
                                            </div>
                                        @endif
                                    @elseif ($column === 'long_description' || $column === 'short_description' || $column === 'meta_description')
                            ...
                            <td style="border-right: none">
                                <button wire:click.prevent="confirmItemRemoval({{ $article->id }})"
                                            ...
                                            @elseif (
                                                ...
                                                @if ($article->$column)
                                                    <p>
                                                        <bold>{{ $column }}:</bold>
                                                    <div class="checkbox--secondary disabled">
                                                        <input type="checkbox" id="disabled3{{ $nr }}" disabled checked>
                                                        <label id="disabled3{{ $nr }}"></label>
                                                    </div>
                                                    </p>
                                                @else
                                                    <p>
                                                        <bold>{{ $column }}:</bold>
                                                    <div class="checkbox--secondary disabled">
                                                        <input type="checkbox" id="disabled4{{ $nr }}" disabled>
                                                        <label id="disabled4{{ $nr }}"></label>
                                                    </div>
                                                    </p>
                                                @endif
                                            @else
